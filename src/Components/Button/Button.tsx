...
const Button: React.FC<ButtonProps> = (props) => {
  const { text, type, onClick, disabled = false, loading = false } = props
  let { className = '' } = props
  let textClassName = ''
  ...
  return (
    <button
      className={`${Styles.container} ${StdFlex.Col} ${StdAlign.Center} ${StdJustify.Center} ${StdPadding.Y6} ${StdPadding.X12} ${className}`}
